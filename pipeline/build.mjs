...
// Nairobi: the matatu network from the Digital Matatus GTFS (2019 survey,
// University of Nairobi C4D Lab / Columbia CSUD / MIT), 136 route entries
// on one road graph — every line navy, nothing on rails. See MODES.
...
// ONE feed, ONE cfg: the matatu network of Nairobi as Digital Matatus mapped
// it — the University of Nairobi's C4D Lab, Columbia's CSUD and MIT's Civic
// Data Design Lab rode every route with a phone in 2013–2019 and published
// the result as GTFS (digitalmatatus.com; the copy here is the DT4A GitLab
// one, the same data the ArcGIS "Nairobi Bus Routes (GTFS)" feature service
// wraps). 136 route entries, 4 284 stops, shapes for every route, one
// agency ("Approved SACCOs"). No timetable to speak of — frequencies.txt —
// which the drawn network does not need.
//
// LINE KEYS: the matatu numbers as the feed prints them — 4, 17B, 46, 105,
// 111, 1961K, "16/62" — and a "_2" suffix marks a second variant of the same
// number (11_2, 17B_2, 3560_2), which merges into its line by the key.
// Every matatu is a bus here: family navy.
const LBL = new Map();

const NIGHT = /^$/; // no night network in the feed
const TROLLEYS = new Set();
const lineRank = (k) => (TROLLEYS.has(k) ? 0 : NIGHT.test(k) ? 2 : 1);
const isRailTrunk = () => false;

const nairobiName = (n) => n.replace(/\s+/g, ' ').trim();
const matatuKey = (sn) => {
  const s = (sn || '').trim().replace(/_\d+$/, '');
  return s || null;
};

const MODES = [{
  mode: 'bus', label: 'matatus', graphMode: 'road',
  // the city and its satellite towns — Limuru, Kiambu, Ruiru, Thika Road,
  // Kitengela, Ngong, Kiserian — 5 × 5 tiles cut out of the Geofabrik Kenya
  // extract (see pipeline/pbf-tiles.py); merged at load, ways deduped by id
  osmFiles: Array.from({ length: 25 }, (_, i) => `data/osm/tiles/t${i + 1}.json`),
  color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['46']),
  feeds: [
    { tag: 'dm', dir: 'data/gtfs', routeTypes: ['3'], mapKey: matatuKey, nameFix: nairobiName },
  ],
}];
